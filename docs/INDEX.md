...
| `ErrorBoundary.tsx`       | React error boundary for graceful failures (reports to Sentry)                           |
...
| `SentryInit.tsx`          | Client component that initializes Sentry + Firebase Analytics on mount (renders nothing) |
...
| File               | Purpose                                                           |
| ------------------ | ----------------------------------------------------------------- |
| `AuthContext.tsx`  | Firebase Anonymous Auth, provides `uid`; sets Sentry user context |
...
| File                         | Purpose                                                                                                                                                                            |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `useRtdbRoom.ts`             | Main room hook — composes `room/*` hooks, returns grouped API (`state`, `connection`, `chat`, `actions`); tracks room join and game completion analytics; sets Sentry room context |
...
| `useGameActions.ts`       | Game action handlers (vote, reveal, clue, start, pause, etc.); tracks game start, rematch, and abandoned game analytics |
...
| `analytics.ts`     | Firebase Analytics wrapper — event tracking helpers (room joins, game starts, completions, rematches) |
| `sentry.ts`        | Sentry error tracking — init, `captureException`, user/room context, breadcrumbs                      |
...
| File | Purpose |
|------|---------|
| `cleanup-rooms.js` | Delete rooms older than N hours (requires Firebase Admin credentials) |
| `cleanup-orphaned-public-rooms.js` | Remove orphaned `/publicRooms` entries |
| `upload-sourcemaps.sh` | Upload source maps to Sentry after build, then delete `.map` files from `out/` |
