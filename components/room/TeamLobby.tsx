          ...
          <div className="mt-6 bg-surface rounded-xl p-4 border border-border">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
              {players.map((player) => {
                ...
                const showKick = canKickPlayer(player.id);
                ...
                return (
                  <div 
                    ...
                    <div className={`font-medium flex items-center gap-1.5 sm:gap-2 ${
                      ...
                      <span className="truncate text-sm sm:text-base">{player.name}{player.id === currentPlayer?.id ? " (you)" : ""}</span>
                      {player.connected === false && (
                        <span className="text-[10px] sm:text-xs uppercase tracking-wide text-muted shrink-0">off</span>
                      )}
                    </div>
                    {/* Row 2: Role */}
                    {player.team && player.role && (
                      <div className={`text-xs sm:text-sm mt-1 ml-6 sm:ml-9 ${
