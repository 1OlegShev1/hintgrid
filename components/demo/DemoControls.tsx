...
export default function DemoControls({ state, controls, onClose }: DemoControlsProps) {
  ...
  return (
    <div className="shrink-0 border-b border-border bg-surface/80 backdrop-blur-sm">
      {/* Main controls row */}
      <div className="flex items-center gap-1.5 sm:gap-3 md:gap-4 px-2 sm:px-4 md:px-6 py-1.5 sm:py-2 md:py-2.5">
        {/* Left: Demo badge + playback */}
        <div className="flex items-center gap-1.5 sm:gap-2.5 md:gap-3">
          {/* Demo badge */}
          <span className="hidden sm:inline-flex items-center gap-1.5 text-sm md:text-base font-semibold text-primary">
          ...
          {/* Divider */}
          <div className="hidden sm:block w-px h-5 md:h-6 bg-border" />
          ...
          {!isComplete ? (
            <>
              <button
                ...
                className="p-1.5 sm:p-2 md:p-2.5 rounded-lg hover:bg-muted/20 transition-colors text-foreground"
              ...
              <button
                ...
                className={`
                  text-xs md:text-sm font-bold px-1.5 sm:px-2 md:px-2.5 py-0.5 sm:py-1 rounded-md transition-colors
                  ${speed === 2
            ...
            <button
              ...
              className="text-sm md:text-base font-medium text-primary hover:text-primary/80 transition-colors"
            >
              Replay
            </button>
          )}
        ...
        <div className="flex-1 flex items-center justify-center min-w-0">
          {phase === "intro" ? (
            <span className="text-xs sm:text-sm md:text-base text-muted truncate">Introduction</span>
          ) : phase === "gameOver" || phase === "gameOverReveal" ? (
            <span className="text-xs sm:text-sm md:text-base font-semibold text-foreground">Game Over!</span>
          ) : (
            <div className="flex items-center gap-1.5 sm:gap-2">
              <span className="text-xs sm:text-sm md:text-base text-muted whitespace-nowrap">
                Turn {turnIndex + 1}/{totalTurns}
              </span>
              <div className="hidden sm:flex gap-1 md:gap-1.5">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 shrink-0">
          {!isComplete && phase !== "intro" && (
            <button
              ...
              className="hidden sm:flex items-center text-xs md:text-sm font-medium rounded-full overflow-hidden border border-border"
              aria-label={`Switch to ${perspective === "hinter" ? "Seeker" : "Hinter"} view`}
            >
              <span
                className={`px-2.5 md:px-3 py-1 md:py-1.5 transition-colors ${
                  perspective === "hinter"
